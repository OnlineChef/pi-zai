...
import { inferEndpoint } from "./state.js";
const CONNECTION_ERROR_PATTERN = /connection.?error|connection.?refused|connection.?lost|fetch failed|network.?error|recv failure|reset before headers|socket hang up|timed? out|timeout|terminated|upstream.?connect/i;
const RECOMMENDED_AGENT_RETRIES = 5;
const RECOMMENDED_PROVIDER_RETRIES = 2;
export function isConnectionErrorMessage(message) {
    if (!message)
        return false;
    return CONNECTION_ERROR_PATTERN.test(message);
}
export function readPiRetrySettings() {
...
export function formatRetrySettingsAdvice(settings) {
    const hints = [];
    if (!settings.enabled) {
    ...
    if (settings.providerMaxRetries < RECOMMENDED_PROVIDER_RETRIES) {
        hints.push(`Add SDK retries for transient drops: retry.provider.maxRetries = ${RECOMMENDED_PROVIDER_RETRIES} (now ${settings.providerMaxRetries})`);
    }
    if (hints.length === 0)
        return undefined;
    return hints.join("; ");
}
export function formatRecommendedRetrySettingsJson() {
...
export function formatConnectionErrorHint(model) {
...
export async function probeChatEndpoint(baseUrl, apiKey, attempts = 3) {
    let ok = 0;
    let fail = 0;
    const latencyMs = [];

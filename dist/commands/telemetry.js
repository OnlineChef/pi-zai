import { getMetricsStorage } from "../state.js";
...
export function registerZaiTelemetryCommand(pi, deps) {
        ...
        handler: async (args, ctx) => {
            ...
            switch (action) {
                case "status": {
                    ...
                    lines.push("", "Enable: set zai.telemetry.mode to aggregate in settings.json, then /zai-telemetry enable", "Disable: /zai-telemetry disable");
                ...
                case "enable": {
                    if (config.telemetryMode !== "aggregate") {
                        ctx.ui.notify('Set "zai": { "telemetry": { "mode": "aggregate" } } in settings.json, /reload, then run /zai-telemetry enable again.', "warning");
                        return;
                    }
                    ...
                    const confirmed = await ctx.ui.confirm("Enable anonymous telemetry?", "Upload anonymous daily aggregate usage to Online Chef Groep? No prompts, code, paths, or fingerprints are sent.");
                    if (!confirmed)
                        return;
                    writeTelemetryConsent();
                    ctx.ui.notify("Telemetry enabled. Completed UTC days upload on session start or via /zai-telemetry sync.", "info");
                    return;
                }
                case "disable":
                ...
                case "upload": {
                    ...
                    if (result.ok) {
                        ctx.ui.notify(`Uploaded aggregate for ${day}${result.status ? ` (HTTP ${result.status})` : ""}.`, "info");
                        return;
                    }
